...
def build_clean_first_page(page_width, page_height):
    ...
    pdf_canvas.setFillColor(GRAY)
    contact_text = "010-7742-1623  |  xhxhahs2@gmail.com  |  경기 하남시  |"
    pdf_canvas.drawString(44.3, 723, contact_text)
    contact_width = pdfmetrics.stringWidth(contact_text, "Malgun", 8.7)
    pdf_canvas.setFont("MalgunBold", 8.7)
    pdf_canvas.setFillColor(TEAL)
    pdf_canvas.drawString(44.3 + contact_width + 6, 723, "github.com/kjongrok")
    ...
    competencies = [
        ...
        "FastAPI·BGE-M3·LLM·Spring 규칙 판정을 분리한 근거 기반 RAG 기능 설계",
    ...
    draw_section_title(pdf_canvas, "WORK EXPERIENCE", 369, page_width)
    draw_entry_heading(pdf_canvas, "㈜시스게이트 · 한림대학교의료원(강동) 전산 운영", "2019.08 – 2023.06 · OA 운영 및 사용자 지원", 342)
    experience_bullets = [
    ...
    draw_section_title(pdf_canvas, "EDUCATION", 241, page_width)
    draw_entry_heading(pdf_canvas, "KG IT BANK · 핀테크 서비스를 위한 풀스택 개발자 양성 과정", "2024.08 – 2025.02", 214)
    draw_body_line(pdf_canvas, "Java, Spring Boot, SQL, JavaScript 기반 웹 서비스 설계와 팀 프로젝트 수행", 193, size=8.5)
    draw_entry_heading(pdf_canvas, "여주대학교 · 컴퓨터정보과", "2016.03 – 2020.02 · 졸업", 169)
...
def build_clean_second_page(page_width, page_height):
    ...
    y_position = 772
    draw_entry_heading(pdf_canvas, "BidMatch · AI 기반 공공입찰 맞춤 추천·자격진단 서비스", "2026.07.15 – 2026.08.05 · 5인 팀 PL", y_position)
    y_position = draw_wrapped_text(pdf_canvas, "나라장터 공고와 기업정보를 연결해 맞춤 공고 추천, 근거 기반 자가 자격진단, 알림과 고객센터를 제공하는 서비스", y_position - 18, color=GRAY)
    bidmatch_bullets = [
        ...
        "BGE-M3 근거 검색, LLM 유형 분류, Spring 규칙 판정을 결합해 AI가 최종 자격 판정을 독점하지 않도록 설계",
        "병렬 개발 중 Flyway 버전·체크섬·레거시 스키마 충돌을 적용 이력을 보존하는 후속 마이그레이션으로 해결",
        "5개 저장소 118개 비병합 커밋 기여, AI pytest 242건 및 Backend·Frontend 최종 빌드/테스트 통과",
    ]
    for bullet_text in bidmatch_bullets:
        y_position = draw_wrapped_text(pdf_canvas, bullet_text, y_position - 2, bullet=True)
    y_position = draw_wrapped_text(pdf_canvas, "기술: Spring Boot · FastAPI · React · PostgreSQL · Redis · BGE-M3 · RAG · LLM · Flyway", y_position - 2, color=TEAL, size=8.0)

    projects = [
        (
            "지능형 교통 관제 시스템",
            "2026.07.04 – 2026.07.07 · 개인",
            [
        ...
        (
            "항공편 지연 예측",
            "2026.06.21 – 2026.06.24 · 개인",
            [
                "약 25만 건 데이터를 전처리하고 XGBoost 등 모델을 비교해 불균형 데이터의 지연 Recall 중심으로 평가",
                "전처리·모델을 scikit-learn Pipeline으로 패키징해 Streamlit에 연결, Recall 65%·ROC-AUC 0.785 기록",
            ],
            None,
        ),
        (
            "맞춤 공고 자동 수집·메일 알림 프로토타입",
            "2026.06.14 – 2026.06.20 · 팀 PL",
            [
    ...
    portfolio_lines = [
        ...
        ("BidMatch Backend", "https://github.com/aiHuman1Team/back-end"),
        ("BidMatch AI", "https://github.com/aiHuman1Team/ai"),
        ("BidMatch Frontend", "https://github.com/aiHuman1Team/front-end"),
    ]
    for label, url in portfolio_lines:
        ...
        pdf_canvas.setFont("Malgun", 8.4)
        pdf_canvas.drawString(44.3 + label_width + 8, y_position, url)
        y_position -= 13
